...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/mangaku07/allow/main/permission/access > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/mangaku07/allow/main/permission/access | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/mangaku07/allow/main/permission/access | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
...
IPs=$(cat /etc/v2ray/domain)
fi
[[ ! -d /var/lib/premium-script ]] && exit 0
...
IPVPS=`curl -sS ip-api.com | grep -w "query" | awk '{print $3}' | cut -d'"' -f2 | tee -a /etc/afak.conf`
    ...
    msg+="<code>ISP Nama      : $ISP\n"
    msg+="City          : $CITY\n"
    msg+="Time          : $WKT\n"
    msg+="IP Address    : $IPVPS</code>\n"
    ShellBot.sendMessage --chat_id ${message_chat_id[$id]} \
        --text "$msg" \
        --parse_mode html
    return 0
...
info_port() {
    [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]] || [[ "$(grep -wc ${callback_query_from_id} $User_Active)" != '0' ]] && {
        ...
        portsshws=$(grep -w "SSH_WS" /root/log-install.txt | awk '{print $1}')
        portovpn=$(grep -w " OpenVPN" /root/log-install.txt | awk '{print $4,$5,$6,$7,$8,$9,$10,$11}')
        ...
        wsssl=`cat /root/log-install.txt | grep -w "SSL_WS" | cut -d: -f2 | awk '{print $1}'`
...
EOF
    ...
    [[ "$t_time" == '1' ]] && hrs="hour" || hrs="hours"
        ...
        portsshws=`cat /root/log-install.txt | grep -w "SSH_WS" | cut -d: -f2 | awk '{print $1}'`
        wsssl=`cat /root/log-install.txt | grep -w "SSL_WS" | cut -d: -f2 | awk '{print $1}'`
        OhpSSH=`cat /root/log-install.txt | grep -w "OHP_SSH" | cut -d: -f2 | awk '{print $1}'`
        OhpDB=`cat /root/log-install.txt | grep -w "OHP_Dropbear" | cut -d: -f2 | awk '{print $1}'`
        OhpOVPN=`cat /root/log-install.txt | grep -w "OHP_OpenVPN" | cut -d: -f2 | awk '{print $1}'`
...
sed -i '/#tls$/a\### '"$userna $exp"'\
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$userna""'"' /etc/$raycheck/config.json
sed -i '/#none$/a\### '"$userna $exp"'\
},{"id": "'""$uuid""'","alterId": '"0"',"email": "'""$userna""'"' /etc/$raycheck/none.json
...
uuid=$(cat /proc/sys/kernel/random/uuid)
sed -i '/#tls$/a\### '"$userna $exp"'\
},{"id": "'""$uuid""'","email": "'""$userna""'"' /etc/$raycheck/vless.json
sed -i '/#none$/a\### '"$userna $exp"'\
},{"id": "'""$uuid""'","email": "'""$userna""'"' /etc/$raycheck/vnone.json

vlesslink1="vless://${uuid}@${domain}:$tls?path=/zivestores%26security=tls%26encryption=none%26type=ws#${userna}"
vlesslink2="vless://${uuid}@${domain}:$none?path=/zivestores%26encryption=none%26type=ws#${userna}"
...
$userna * $passw *
EOF
echo -e "### $userna $exp">>"/var/lib/premium-script/data-user-sstp"
...
func_del_sstp() {
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^###" /var/lib/premium-script/data-user-sstp | cut -d ' ' -f 3 )
        sed -i "/^### $userna $exp/d" /var/lib/premium-script/data-user-sstp
...
list_member_sstp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/premium-script/data-user-sstp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/premium-script/data-user-sstp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
echo -e "### $userna $exp">>"/var/lib/premium-script/data-user-l2tp"
...
func_del_l2tp() {
    ...
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^### " /var/lib/premium-script/data-user-l2tp | cut -d ' ' -f 3 )
        sed -i '/^"'"$userna"'" l2tpd/d' /etc/ppp/chap-secrets
        sed -i '/^'"$userna"':\$1\$/d' /etc/ipsec.d/passwd
        sed -i "/^### $userna $exp/d" /var/lib/premium-script/data-user-l2tp
        chmod 600 /etc/ppp/chap-secrets* /etc/ipsec.d/passwd*
        datata=$(find /etc/.maAsiss/ -name $userna)
        for accc in "${datata[@]}"
        do
...
list_member_l2tp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/premium-script/data-user-l2tp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/premium-script/data-user-l2tp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
echo -e "### $userna $exp">>"/var/lib/premium-script/data-user-pptp"
local env_msg
env_msg="━━━━━━━━━━━━━━━━━━━━━\n<b>         🔸 PPTP ACCOUNT 🔸 </b>\n━━━━━━━━━━━━━━━━━━━━━\n"
env_msg+="IP : $IPs\n"
env_msg+="Username : $userna\n"
...
func_del_pptp() {
    ...
    userna=$1
    if [[ "${message_from_id[$id]}" = "$Admin_ID" ]]; then
        exp=$(grep -E "^### " /var/lib/premium-script/data-user-pptp | cut -d ' ' -f 3 )
        sed -i '/^"'"$userna"'" pptpd/d' /etc/ppp/chap-secrets
        sed -i "/^### $userna $exp/d" /var/lib/premium-script/data-user-pptp
        chmod 600 /etc/ppp/chap-secrets* /etc/ipsec.d/passwd*
        datata=$(find /etc/.maAsiss/ -name $userna)
        for accc in "${datata[@]}"
        do
...
list_member_pptp() {
   if [[ "${callback_query_from_id[$id]}" == "$Admin_ID" ]]; then
      _result=$(grep -wE "^###" "/var/lib/premium-script/data-user-pptp" | cut -d ' ' -f2 | column -t | sort | uniq | wc -l)
      _results=$(grep -wE "^###" "/var/lib/premium-script/data-user-pptp" | cut -d ' ' -f2 | column -t | sort | uniq )
   elif [[ "${callback_query_from_id[$id]}" != "$Admin_ID" ]]; then
...
linktrgo="trojan-go://${uuidR}@${domain}:${trgo}/?sni=${domain}%26type=ws%26host=${domain}%26path=/TrGo%26encryption=none#${userna}"
...
ShellBot.InlineKeyboardButton --button 'menu_adm_ser' --line 1 --text '• Menu VMess •' --callback_data '_menuv2ray'
ShellBot.InlineKeyboardButton --button 'menu_adm_ser' --line 2 --text '• Menu Trojan •' --callback_data '_menutrojan'
...
ShellBot.InlineKeyboardButton --button 'menu_re_ser' --line 1 --text '• VMess •' --callback_data '_res_v2ray_menus'
ShellBot.InlineKeyboardButton --button 'menu_re_ser' --line 2 --text '• Trojan •' --callback_data '_res_trojan_menus'
...
ShellBot.InlineKeyboardButton --button 'menu_re_main' --line 2 --text '📚 Info Port 📚' --callback_data '_pports'
ShellBot.InlineKeyboardButton --button 'menu_re_main' --line 3 --text '📁 Close Menu 📁' --callback_data 'closesss'
...
tls="$(cat /root/log-install.txt | grep -w "Vmess TLS" | cut -d: -f2|sed 's/ //g')"
none="$(cat /root/log-install.txt | grep -w "Vmess None TLS" | cut -d: -f2|sed 's/ //g')"
tls1="$(cat /root/log-install.txt | grep -w "Vless TLS" | cut -d: -f2|sed 's/ //g')"
none1="$(cat /root/log-install.txt | grep -w "Vless None TLS" | cut -d: -f2|sed 's/ //g')"
trgo="$(cat /root/log-install.txt | grep -w "Trojan Go" | cut -d: -f2|sed 's/ //g')"
tr="$(cat /root/log-install.txt | grep -w "Trojan " | cut -d: -f2|sed 's/ //g')"
OhpSSH=`cat /root/log-install.txt | grep -w "OHP_SSH" | cut -d: -f2 | awk '{print $1}'`
OhpDB=`cat /root/log-install.txt | grep -w "OHP_Dropbear" | cut -d: -f2 | awk '{print $1}'`
...
dx=$(ls /etc/.maAsiss/public_mode --ignore='settings' | wc -l)
   ...
   env_msg+="•VMess/VLess WS TLS : $tls/$tls1\n"
   env_msg+="•VMess/VLess WS Non TLS : $none/$none1\n"
   env_msg+="━━━━━━━━━━━━━━━━━━━━━\n"
...
ShellBot.InlineKeyboardButton --button 'pub_menu' --line 2 --text '• VLESS •' --callback_data 'vless'
ShellBot.InlineKeyboardButton --button 'pub_menu' --line 2 --text '• TROJAN •' --callback_data 'trojan'
#ShellBot.InlineKeyboardButton --button 'pub_menu' --line 2 --text '• TROJAN-GO •' --callback_data 'trgo'
            ...
            if [[ ${message_reply_to_message_message_id[$id]} ]]; then
                ...
                if ((${message_text[$id]} >= 1 && ${message_text[$id]} <= 30)); then
                    ...
                    [[ "(grep -w ${message_text[$id]} /etc/passwd)" = '0' ]] && {
                        ...
                        ossl=`cat /root/log-install.txt | grep -w " OpenVPN" | cut -f2 -d: | awk '{print $6}'`
                        opensh=`cat /root/log-install.txt | grep -w "OpenSSH" | cut -f2 -d: | awk '{print $1,$2}'`
                        db=`cat /root/log-install.txt | grep -w "Dropbear" | cut -f2 -d: | awk '{print $1,$2}'`
                        ssl="$(cat /root/log-install.txt | grep -w "Stunnel4" | cut -d: -f2)"
                        sqd="$(cat /root/log-install.txt | grep -w "Squid" | cut -d: -f2)"
                        ovpn="$(netstat -nlpt | grep -i openvpn | grep -i 0.0.0.0 | awk '{print $4}' | cut -d: -f2)"
