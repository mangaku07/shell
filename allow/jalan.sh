...
BURIQ () {
    curl -sS https://zikristore.my.id/izin > /root/tmp
...
Name=$(curl -sS https://zikristore.my.id/izin | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://zikristore.my.id/izin | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
