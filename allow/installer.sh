...
surat=$(curl -sS https://raw.githubusercontent.com/mangaku07/allow/main/permission/allow-bot | grep -w $ipes | awk '{print $4}'  )
if [[ "$surat" = "true" ]]; then
echo -ne
else
...
dircreate
        ...
        echo -e "Shell Bot Telegram Panel Installer
        "
        [[ ! -f /root/ResBotAuth ]] && {
        ...
        echo -ne "Username admin panel use '@' [Ex: @ch1431] : "
        read admin_pnl
        [[ -z $admin_pnl ]] && admin_pnl="@ch1431"
        echo ""
        echo -ne "Limit trial for reseller create user trial [default:1] : "
        read limit_pnl
        [[ -z $limit_pnl ]] && limit_pnl="1"
        echo ""
        echo -ne "Your name store [dafult: ANDALAS-STORE] : "
        read store_pnl
        [[ -z $store_pnl ]] && store_pnl="ANDALAS-STORE"
        echo ""
...
EOF
        ...
        fun_bot1() {
			...
			[[ "$(grep -wc "shell_bot" "/etc/rc.local")" = '0' ]] && {
			    sed -i '$ i\screen -dmS shell_bot bbt' /etc/rc.local >/dev/null 2>&1
			}
        }
        screen -dmS shell_bot bbt >/dev/null 2>&1
        fun_bot1
        [[ $(ps x | grep "shell_bot" | grep -v grep | wc -l) != '0' ]] && echo -e "\nBot successfully activated !" || echo -e "\nError1! Information not valid !"
        sleep 7
        menu
        } || {
       clear
        echo -e "Info...\n"
        fun_bot2() {
            screen -r -S "shell_bot" -X quit >/dev/null 2>&1
            [[ $(grep -wc "shell_bot" /etc/rc.local) != '0' ]] && {
                sed -i '/shell_bot/d' /etc/rc.local
            }
            rm -f /etc/.maAsiss/bot.conf
            sleep 5
        }
        fun_bot2
        echo -e "\nShell Bot Telegram berhenti!"
        sleep 7
        menu
    }
}
